 ...
 */
if ( !function_exists('movedo_grve_element_navigation') ) {
	function movedo_grve_element_navigation( $navigation_type = 0, $navigation_color = 'dark' ) {
		...
		if ( 0 != $navigation_type ) {
			...
			$output .= '<span class="screen-reader-text">' . esc_html( 'Previous', 'movedo' ) . '</span>';
			$output .= '</div>';
			$output .= '<div class="grve-carousel-next">';
			$output .= '<i class="' . esc_attr( $icon_nav_next ) . '"></i>';
			$output .= '<span class="screen-reader-text">' . esc_html( 'Next', 'movedo' ) . '</span>';
			$output .= '</div>';
			$output .= '</div>';
			$output .= '</div>';
		}
 ...
 */
if ( !function_exists('movedo_grve_print_gallery_slider') ) {
 ...
 */
if ( !function_exists('movedo_grve_print_media_video') ) {
	function movedo_grve_print_media_video( $video_mode, $video_webm, $video_mp4, $video_ogv, $video_embed, $video_poster = '' ) {
		...
		if( empty( $video_mode ) ) {
			...
			if ( !empty( $video_webm ) || !empty( $video_mp4 ) || !empty( $video_ogv ) ) {
				...
				echo '<video ' . movedo_grve_print_media_video_settings( $video_settings ) . ' >';
...
if ( !function_exists('movedo_grve_get_image_size') ) {
...
if ( !function_exists('movedo_grve_get_fallback_image_attr') ) {
...
if ( !function_exists('movedo_grve_get_fallback_image') ) {
