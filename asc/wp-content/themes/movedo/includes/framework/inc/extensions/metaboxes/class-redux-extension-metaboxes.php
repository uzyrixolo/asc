					...
					<div class="redux-sidebar">
						<ul class="redux-group-menu">
							...
							foreach ( $sections as $s_key => $section ) {
								...
								echo redux_custom_section_menu( $this, $s_key, $section, '_box_' . $metabox['id'], $sections ); // phpcs:ignore WordPress.Security.EscapeOutput
							}
							?>
						</ul>
					</div>
							...
							foreach ( $section['fields'] as $field ) {
								...
								if ( $sidebar ) {
									if ( ! ( isset( $metabox['args']['sections'] ) && 1 === count( $metabox['args']['sections'] ) && isset( $metabox['args']['sections'][0]['fields'] ) && 1 === count( $metabox['args']['sections'][0]['fields'] ) ) && isset( $field['title'] ) ) {
										echo '<th scope="row">';
										if ( ! empty( $th ) ) {
											echo str_replace('redux_field_mod_tom', 'redux_field_mod_tom_fixed', $th);
										}
										echo '</th>';
										echo '<td>';
									}
