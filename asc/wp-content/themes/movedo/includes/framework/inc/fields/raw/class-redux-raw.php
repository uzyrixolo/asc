		...
		public function render() {
			...
			if ( ! empty( $this->field['content'] ) && isset( $this->field['content'] ) ) {
				if ( isset( $this->field['markdown'] ) && true === $this->field['markdown'] && ! empty( $this->field['content'] ) ) {
					...
					echo wp_kses_post( $parsedown->text( $this->field['content'] ) ); // phpcs:ignore WordPress.Security.EscapeOutput
				} else {
					echo wp_kses_post( $this->field['content'] ); // phpcs:ignore WordPress.Security.EscapeOutput
				}
			}
