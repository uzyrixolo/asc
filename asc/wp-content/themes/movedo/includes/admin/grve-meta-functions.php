...
function movedo_grve_page_options_box( $post ) {
	...
	$movedo_grve_anchor_device_layout = get_post_meta( $post->ID, '_movedo_grve_anchor_device_layout', true );
				...
				<div id="grve-page-option-tab-extras" class="grve-tab-item">
					<?php
						...
						movedo_grve_print_admin_option(
							array(
								'type' => 'select',
								'name' => '_movedo_grve_anchor_device_layout',
								'id' => '_movedo_grve_anchor_device_layout',
								'value' => $movedo_grve_anchor_device_layout,
								'options' => array(
									'' => esc_html__( 'Convert to Hidden Menu', 'movedo' ),
									'none' => esc_html__( 'Same as desktop', 'movedo' ),
								),
								'label' => array(
									"title" => esc_html__( 'Anchor Responsive Layout', 'movedo' ),
									"desc" => esc_html__( 'Select anchor responsive layout ( on 1024px and below ).', 'movedo' ),
								),
							)
						);
	...
	if ( isset( $_POST['_movedo_grve_nonce_page_save'] ) && wp_verify_nonce( $_POST['_movedo_grve_nonce_page_save'], 'movedo_grve_nonce_page_save' ) ) {
		...
		if ( movedo_grve_check_permissions( $post_id ) ) {
			...
			$movedo_grve_page_options = array (
				...
				array(
					'name' => 'Navigation Anchor Device Layout',
					'id' => '_movedo_grve_anchor_device_layout',
				),
				array(
					'name' => 'Theme Loader',
