...
if ( !function_exists('movedo_grve_print_anchor_menu') ) {
	function movedo_grve_print_anchor_menu( $mode = 'page') {
		...
		if ( movedo_grve_is_woo_shop() ) {
			...
			$anchor_device_layout = movedo_grve_post_meta_shop( '_movedo_grve_anchor_device_layout' );
		} else {
			$anchor_nav_menu = movedo_grve_post_meta( '_movedo_grve_anchor_navigation_menu' );
			$anchor_device_layout = movedo_grve_post_meta( '_movedo_grve_anchor_device_layout' );
		}
		...
		if ( !empty( $anchor_nav_menu ) ) {
			...
			if ( '' == $anchor_device_layout ) {
				$movedo_grve_anchor_classes[] = 'grve-anchor-responsive-layout';
			}
...
if ( !function_exists('movedo_grve_print_header_top_bar_socials') ) {
	function movedo_grve_print_header_top_bar_socials( $options ) {
		...
		$social_list_extended = $movedo_grve_social_list_extended;
		$social_options = movedo_grve_option('social_options');
		$social_rel = movedo_grve_social_rel();
		if ( !empty( $options ) && !empty( $social_options ) ) {
			...
			foreach ( $social_options as $key => $value ) {
				if ( isset( $social_list_extended[$key] ) && isset( $options[$key] ) && 1 == $options[$key] && $value ) {
					if ( 'skype' == $key ) {
						echo '<li><a href="' . esc_url( $value, array( 'skype', 'http', 'https' ) ) . '" class="' . esc_attr( $social_list_extended[$key]['class'] ) . '" aria-label="' . esc_attr( $social_list_extended[$key]['title'] ) . '"></a></li>';
					} else {
						echo '<li><a href="' . esc_url( $value ) . '" target="_blank" rel="' . implode( ' ', $social_rel ) . '" class="' . esc_attr( $social_list_extended[$key]['class'] ) . '" aria-label="' . esc_attr( $social_list_extended[$key]['title'] ) . '"></a></li>';
					}
				}
			}
			?>
						...
						if( !empty( $value ) && '0' != $value && movedo_grve_check_header_elements_visibility( $key ) ) {
							...
							<?php
									...
									if ( !empty( $header_social_options ) && !empty( $social_options ) ) {
										...
										foreach ( $social_options as $key => $value ) {
											if ( isset( $social_list_extended[$key] ) && isset( $header_social_options[$key] ) && 1 == $header_social_options[$key] && $value ) {
												if ( 'skype' == $key ) {
													echo '<li class="grve-header-element"><a href="' . esc_url( $value, array( 'skype', 'http', 'https' ) ) . '" aria-label="' . esc_attr( $social_list_extended[$key]['title'] ) . '"><span class="grve-item"><i class="' . esc_attr( $social_list_extended[$key]['class'] ) . '"></i></span></a></li>';
												} else {
													echo '<li class="grve-header-element"><a href="' . esc_url( $value ) . '" target="_blank" rel="' . implode( ' ', $social_rel ) . '" aria-label="' . esc_attr( $social_list_extended[$key]['title'] ) . '"><span class="grve-item"><i class="' . esc_attr( $social_list_extended[$key]['class'] ) . '"></i></span></a></li>';
												}
											}
										}
...
<?php
										foreach ( $social_options as $key => $value ) {
											if ( isset( $social_list_extended[$key] ) && isset( $header_social_options[$key] ) && 1 == $header_social_options[$key] && $value ) {
												if ( 'skype' == $key ) {
													echo '<li class="grve-header-responsive-element"><a href="' . esc_url( $value, array( 'skype', 'http', 'https' ) ) . '" aria-label="' . esc_attr( $social_list_extended[$key]['title'] ) . '"><span class="grve-item"><i class="' . esc_attr( $social_list_extended[$key]['class'] ) . '"></i></span></a></li>';
												} else {
													echo '<li class="grve-header-responsive-element"><a href="' . esc_url( $value ) . '" target="_blank" rel="' . implode( ' ', $social_rel ) . '" aria-label="' . esc_attr( $social_list_extended[$key]['title'] ) . '"><span class="grve-item"><i class="' . esc_attr( $social_list_extended[$key]['class'] ) . '"></i></span></a></li>';
												}
											}
										}
?>
