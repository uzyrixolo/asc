<?php
	...
	if( !empty ( $separator_top ) ) {
		echo '<div class="grve-separator-top">' . $separator_svg_top . '</div>';
	}
	echo '<div class="grve-container">';
	echo '<div class="' . esc_attr( $row_css_string ) . '">' . do_shortcode( $content ) . '</div>';
	echo '</div>';
	echo '<div class="grve-background-wrapper">' . $out_video_bg_url . $out_image_bg . $out_video_bg . $out_overlay . $out_pattern . '</div>';
	if( !empty ( $separator_bottom ) ) {
		echo '<div class="grve-separator-bottom">' . $separator_svg_bottom . '</div>';
	}
	echo '</div>';
