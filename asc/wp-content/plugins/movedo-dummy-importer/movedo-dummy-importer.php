...
if ( ! defined( 'MOVEDO_IMPORT_SPLIT_ATTACHMENTS' ) ) {
	define( 'MOVEDO_IMPORT_SPLIT_ATTACHMENTS', true );
}
...
class Movedo_Importer extends WP_Importer {
	...
	function __construct() {
		add_action( 'wp_ajax_movedo_import_attachments', array( $this, 'movedo_import_attachments' ) );
	...
	function movedo_demo_importer_scripts( $hook ) {
		...
		if ( 'admin.php' == $hook ) {
			...
			$grve_import_texts = array(
				...
				'split_enabled' => MOVEDO_IMPORT_SPLIT_ATTACHMENTS,
				'debug_enabled' => MOVEDO_IMPORT_DEBUG,
				...
				'error_text' => esc_html__( "An unexpected error occured:", 'movedo-importer' ),
			);
	...
	function grve_get_demo_data() {
		$grve_dummy_data_list = array(
			array(
				...
				'homepage' => 'Intro',
				'movedo_header_nav'  => 'main-menu',
				'movedo_footer_nav'  => 'footer-menu',
			),
			array(
				'id'   => 'movedo-corporate',
			...
			array(
				...
				'homepage'  => 'Home',
				'movedo_header_nav'  => 'main-nav',
				'movedo_footer_nav'  => 'main-nav',
			),
			array(
				'id'   => 'movedo-blog',
				...
				'homepage'  => 'Home',
				'movedo_header_nav'  => 'main-navigation',
			),
			array(
				'id'   => 'movedo-landing',
			...
			array(
				'id'   => 'movedo-fitness',
				'name' => 'Movedo Fitness Demo',
				'dir'  => 'movedo-fitness',
				'preview'  => 'movedo/movedo-fitness',
				'description'  => esc_html__( 'Premium case study for gyms and personal trainers. Modern and clean design.', 'movedo-importer' ),
				'homepage'  => 'Home',
				'movedo_header_nav'  => 'main-menu',
				'movedo_footer_nav'  => 'footer-menu',
			),
			array(
				'id'   => 'movedo-moments',
				'name' => 'Movedo Moments Demo',
				'dir'  => 'movedo-moments',
				'preview'  => 'movedo/movedo-moments',
				'description'  => esc_html__( 'Simple and clean design to capture unique moments. Recommended for personal use and photographers or freelancers.', 'movedo-importer' ),
				'homepage'  => 'Homepage',
				'movedo_header_nav'  => 'main-menu',
			),
			array(
				'id'   => 'movedo-medical',
				'name' => 'Movedo Medical Demo',
				'dir'  => 'movedo-medical',
				'preview'  => 'movedo/movedo-medical',
				'description'  => esc_html__( 'Medical case study, simple and clean design demonstrating Events Calendar.', 'movedo-importer' ),
				'homepage'  => 'Home',
				'movedo_header_nav'  => 'main-menu',
			),
		);
	...
	function movedo_import_check() {
		$theme_active = $this->is_theme_active();
	...
	function movedo_import_get_errors() {
		...
		if ( !$theme_active ) {
			...
			$importer_info .=  '<br/><br/>';
			$importer_info .=  '<a href="' . esc_url( admin_url( 'themes.php' ) ) . '" target="_blank">' .  esc_html__( "Themes", 'movedo-importer' ) . '</a>';
		} else {
			if ( ! class_exists( 'Redux' ) ) {
				$importer_info .=  '<i class="dashicons dashicons-no"></i> <b>' .  esc_html__( "Importer runs with Redux v4. Movedo Theme needs to be updated!", 'movedo-importer' ) . '</b>';
				$importer_info .=  '<br/>';
			}
		}
	...
	function movedo_import_attachments() {
		...
		if ( isset( $_POST['grve_import_data'] ) ) {
			...
			$theme_check = $this->movedo_import_check();
			if ( $theme_check ) {
				...
				if ( isset( $_POST['grve_import_attachments'] ) && 'true' == $_POST['grve_import_attachments'] ) {
					...
					$importer_info.= esc_html__( "Attachments Imported", 'movedo-importer' ) . " " . $grve_index . "/" . $grve_total . "<br>";
					$grve_changed = true;
				}
				...
				if ( !$grve_importer_error ) {
					if( $grve_finished ) {
						$importer_info.=  '<br/>';
						$importer_info.=  '<i class="dashicons dashicons-yes"></i> ' .  esc_html__( "Attachments:  Imported!", 'movedo-importer' ) . '<br/>';
					}
				} else {
					$importer_info.=  '<br/>';
					$importer_info.=  '<i class="dashicons dashicons-no"></i> <b>' .  esc_html__( "Attachment Import finished with errors!", 'movedo-importer' ) . '</b><br/>';
				}
			} else {
					$grve_importer_error = true;
					$importer_info .= $this->movedo_import_get_errors();
			}
			...
			if ( $theme_check ) {
						...
						if ( is_wp_error( $import_output ) ) {
							...
							if( ! empty( $grve_menus ) ) {
								...
								foreach ( $grve_menus as $grve_menu ) {
									...
									if( isset( $grve_dummy_data['movedo_header_nav'] ) && $grve_dummy_data['movedo_header_nav'] == $grve_menu->slug ) {
										$locations['movedo_header_nav'] = $grve_menu->term_id;
									}
									if( isset( $grve_dummy_data['movedo_footer_nav'] ) && $grve_dummy_data['movedo_footer_nav'] == $grve_menu->slug ) {
							...
							if( isset( $grve_dummy_data['homepage'] ) && !empty( $grve_dummy_data['homepage'] ) ) {
								$homepage = $this->grve_get_page_by_title( $grve_dummy_data['homepage'] );
							}
							if ( $homepage ) {
				...
				if ( !$grve_importer_error ) {
					if( $grve_changed ) {
						...
						$importer_info.=  '<br/><br/>';
						if ( defined('MOVEDO_IMPORT_DEBUG') && MOVEDO_IMPORT_DEBUG ) {
							$importer_info.=  '<a href="' . esc_url( home_url() ) . '" target="_blank">' .  esc_html__( "Visit Site", 'movedo-importer' ) . '</a>';
						} else {
							$importer_info.=  esc_html__( "Redirecting to finish page...", 'movedo-importer' );
						}
					} else {
					...
					$importer_info.=  ' <a href="' . esc_url( home_url( '/' ) ) . '">' .  esc_html__( "Visit Site", 'movedo-importer' ) . '</a>';
					$importer_info.=  '<br/>';
				}
			} else {
					$grve_importer_error = true;
	...
	/**
	 * Additional function to get page by title
 	 */
	function grve_get_page_by_title( $page_title ) {
		$posts = get_posts(
			array(
				'post_type'              => 'page',
				'title'                  => $page_title,
	...
	function grve_import_options( $import_file, $dummy_id = 'movedo'  ) {
		...
		if ( !empty( $import_array ) ) {
			...
			switch ( $dummy_id ) {
				...
				break;
				case 'movedo-fitness':
					$imported_options['logo'] = array( 'url' => get_template_directory_uri() .'/images/logos/' . $dummy_id . '/logo-dark.png', 'width' => '245', 'height' => '100' );
					$imported_options['logo_light'] = array( 'url' => get_template_directory_uri() .'/images/logos/' . $dummy_id . '/logo-light.png', 'width' => '245', 'height' => '100' );
					$imported_options['logo_dark'] = $imported_options['logo'];
				break;
				case 'movedo-moments':
					$imported_options['logo'] = array( 'url' => get_template_directory_uri() .'/images/logos/' . $dummy_id . '/logo-dark.png', 'width' => '230', 'height' => '92' );
					$imported_options['logo_responsive'] = $imported_options['logo_sticky'] = $imported_options['logo_light'] = $imported_options['logo_dark'] = $imported_options['logo'];
				break;
				case 'movedo-medical':
					$imported_options['logo'] = array( 'url' => get_template_directory_uri() .'/images/logos/' . $dummy_id . '/logo-default.png', 'width' => '222', 'height' => '56' );
					$imported_options['logo_responsive'] = $imported_options['logo_sticky'] = $imported_options['logo_light'] = $imported_options['logo_dark'] = $imported_options['logo'];
				break;
				default:
				break;
	...
	function process_posts( $ids_array = array() ) {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				//Remap Categories
				$pattern = '|categories="([^"]*)"|';
				$post['post_content'] = preg_replace_callback($pattern, array( $this, "grve_categories_callback" ), $post['post_content']);
		...
		<div id="grve-importer-welcome-page" class="grve-content-wrapper">
			...
			<div class="grve-importer-content">
				...
				<div class="grve-setting-wrap">
					...
					<div class="grve-setting-inner-wrap">
						<strong class="grve-setting-label"><?php esc_html_e('PHP Memory Limit', 'movedo-importer'); ?><span class="grve-small-code"> (memory_limit) </span></strong>
					<?php
					...
					<div class="grve-setting-inner-wrap">
						<strong class="grve-setting-label"><?php esc_html_e('PHP Max. Post Size', 'movedo-importer'); ?><span class="grve-small-code"> (post_max_size) </span></strong>
					<?php
					...
					<div class="grve-setting-inner-wrap">
						<strong class="grve-setting-label"><?php esc_html_e('PHP Upload Max. Filesize', 'movedo-importer'); ?><span class="grve-small-code"> (upload_max_filesize) </span></strong>
					<?php
					...
					<?php
						...
						echo '</span>';
						if( $is_max_input_vars_min ){
							echo '<span class="grve-setting-content">' . ' ' . esc_html__( '(recommended 2000 or higher)', 'movedo-importer' ).'</span>';
						}
					?>
					</div>
					<div class="grve-setting-inner-wrap">
						<strong class="grve-setting-label"><?php esc_html_e('PHP Max. Execution time', 'movedo-importer'); ?><span class="grve-small-code"> (max_execution_time) </span></strong>
					<?php
						...
						echo '<span class="grve-setting-title">';
						echo esc_html__('Currently:', 'movedo-importer') . ' ' . $max_execution_time;
						echo '</span>';
						if( $is_max_execution_time_min ){
							echo '<span class="grve-setting-content">'. ' ' . esc_html__( '(recommended 300 or higher)', 'movedo-importer' ).'</span>';
						}
					?>
					</div>
					<div class="grve-setting-inner-wrap">
