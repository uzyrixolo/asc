...
if( !function_exists( 'movedo_ext_vce_blog_shortcode' ) ) {
	...
	function movedo_ext_vce_blog_shortcode( $atts, $content ) {
		...
		$args = apply_filters( 'movedo_grve_vce_blog_args', $args, $combined_atts );
							...
							<div class="grve-post-content">
								<div class="grve-post-header">
									...
									if ( 'yes' != $hide_categories ) {
										movedo_ext_vce_print_post_categories();
									}
									?>
									<?php movedo_ext_vce_print_post_title( $blog_mode, $post_format, $post_title_heading_tag, $post_title_heading ); ?>
								</div>
...
<?php
		...
		do_action( 'movedo_grve_vce_blog_no_results' );
		endif;
	...
	function movedo_ext_vce_blog_shortcode_params( $tag ) {
		return array(
			...
			"params" => array(
				...
				array(
					"type" => 'checkbox',
					"heading" => esc_html__( "Hide Categories", "movedo-extension" ),
					"param_name" => "hide_categories",
					"description" => esc_html__( "If selected, blog overview will not show categories.", "movedo-extension" ),
					"value" => array( esc_html__( "Hide Categories.", "movedo-extension" ) => 'yes' ),
					"std" => 'yes',
					"dependency" => array( 'element' => "blog_mode", 'value' => array( 'blog-large', 'blog-small','grid', 'masonry' ) ),
					"group" => esc_html__( "Extras", "movedo-extension" ),
				),
