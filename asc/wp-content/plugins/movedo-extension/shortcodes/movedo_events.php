...
if( !function_exists( 'movedo_ext_vce_events_shortcode' ) ) {
	...
	function movedo_ext_vce_events_shortcode( $attr, $content ) {
		...
		while ( $query->have_posts() ) : $query->the_post();
			...
			$post_id = get_the_ID();
			$item_event_classes = tribe_get_post_class( [ $event_extra_class ], $post_id  );

?>
			<div id="grve-tribe-events-event-<?php the_ID(); ?><?php echo uniqid('-'); ?>" <?php tribe_classes( $item_event_classes ); ?>>
	...
	function movedo_ext_vce_events_shortcode_params( $tag ) {
		return array(
			...
			"params" => array(
				...
				array(
					"type" => "dropdown",
					"heading" => esc_html__( "Exclude Past Events", "movedo-extension" ),
					...
					"std" => 'no',
					"description" => esc_html__( "Select if you want to exclude past events.", "movedo-extension" ),
