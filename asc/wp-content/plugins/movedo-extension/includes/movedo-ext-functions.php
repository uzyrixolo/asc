...
function movedo_ext_vce_increase_heading_reset( $increase_heading_reset, $title_classes ) {
	switch( $increase_heading_reset ) {
		case 'desktop-sm':
			array_push( $title_classes, 'grve-desktop-sm-reset-increase-heading' );
			array_push( $title_classes, 'grve-tablet-reset-increase-heading' );
			array_push( $title_classes, 'grve-tablet-sm-reset-increase-heading' );
			array_push( $title_classes, 'grve-mobile-reset-increase-heading' );
		break;
		case 'tablet':
			array_push( $title_classes, 'grve-tablet-reset-increase-heading' );
			array_push( $title_classes, 'grve-tablet-sm-reset-increase-heading' );
 ...
 */
if ( ! function_exists( 'movedo_ext_get_microdata_allowed_html' ) ) {
	function movedo_ext_get_microdata_allowed_html() {
		return array(
			'span' => array(
				'title' => true,
				'class' => true,
...
function movedo_ext_social_rel() {
	if ( function_exists( 'movedo_grve_social_rel' ) ) {
		return movedo_grve_social_rel();
	} else {
		return array ( 'noopener', 'noreferrer' );
	}
}
...
function movedo_ext_vce_get_button( $button_options ) {
	...
	$button_id = movedo_ext_vce_array_value( $button_options, 'button_id' );
...
function movedo_ext_vce_get_link_attributes( $link = '', $class = '', $style = ''  ) {
	...
	if ( $use_link ) {
		...
		if ( ! empty( $a_target ) ) {
			...
			if ( ! empty( $a_rel ) ) {
				$attributes[] = 'rel="noopener noreferrer ' . esc_attr( trim( $a_rel ) ) . '"';
			} else {
 ...
 function movedo_ext_vce_print_post_loop( $blog_mode, $post_format, $heading_tag, $heading, $auto_excerpt, $excerpt_length, $excerpt_more ) {
	...
	switch( $post_format ) {
		case 'link':
				...
				echo '<a class="grve-post-link" ' . movedo_ext_vce_print_post_link( 'link' ) . ' rel="bookmark" aria-label="' . the_title_attribute( 'echo=0' ) . '"></a>';
		...
		case 'quote':
				...
				echo '<a class="grve-post-link" href="' . esc_url( get_permalink() ) . '" rel="bookmark" aria-label="' . the_title_attribute( 'echo=0' ) . '"></a>';
				movedo_ext_vce_post_bg_image_container( $bg_options );
				echo '<div class="grve-post-content-wrapper">';
				echo '  <div class="grve-post-content">';
				echo '    <div class="grve-post-icon">';
...
function movedo_ext_vce_print_post_title( $blog_mode, $post_format, $heading_tag, $heading ) {
	...
	if( 'leader' == $blog_mode ) {
		the_title( '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><' . tag_escape( $title_tag ) . ' class="grve-post-title grve-post-title-hover grve-' . esc_attr( $heading ) . '" itemprop="headline">', '</' . tag_escape( $title_tag ) . '></a>' );
	} else {
		switch( $post_format ) {
			case 'link':
				if( 'carousel' == $blog_mode ) {
					the_title( '<a ' . movedo_ext_vce_print_post_link( 'link' ) . ' rel="bookmark"><' . tag_escape( $title_tag ) . ' class="grve-post-title grve-' . esc_attr( $heading ) . '" itemprop="headline">', '</' . tag_escape( $title_tag ) . '></a>' );
				} else {
					if( 'auto' == $heading ) {
						$heading = 'h4';
					}
					the_title( '<' . tag_escape( $title_tag ) . ' class="grve-post-title grve-' . esc_attr( $heading ) . '" itemprop="headline">', '</' . tag_escape( $title_tag ) . '>' );
				}
				break;
			case 'quote':
				if( 'carousel' == $blog_mode ) {
 ...
 */
function movedo_ext_vce_print_carousel_media( $carousel_image_mode = 'landscape', $attr = array() ) {
	global $post;
	$post_id = $post->ID;
...
function movedo_ext_vce_print_post_feature_media( $blog_mode, $post_format, $blog_image_mode, $blog_image_prio, $image_atts = array() ) {
...
?>
		<div class="grve-media">
			<a class="grve-item-url" href="<?php echo esc_url( $image_href ); ?>" aria-label="<?php the_title_attribute(); ?>"></a>
...
function movedo_ext_vce_element_navigation( $navigation_type = 0, $navigation_color = 'dark', $navigation_element = 'default' ) {
		...
		if ( 0 != $navigation_type ) {
			...
			$output .= '<span class="screen-reader-text">' . esc_html( 'Previous', 'movedo-extension' ) . '</span>';
			$output .= '</div>';
			$output .= '<div class="grve-carousel-next">';
			$output .= '<i class="grve-icon-nav-right-small"></i>';
			$output .= '<span class="screen-reader-text">' . esc_html( 'Next', 'movedo-extension' ) . '</span>';
			$output .= '</div>';
			$output .= '</div>';
			$output .= '</div>';
		}
 ...
 */
if ( !function_exists( 'movedo_ext_vce_print_post_categories' ) ) {
	function movedo_ext_vce_print_post_categories() {
		...
		if ( !empty( $post_terms ) && !is_wp_error( $post_terms ) ) {
			...
			$args = apply_filters('movedo_ext_post_categories_args', $args);
...
if ( ! function_exists( 'movedo_ext_browser_webkit_check' ) ) {
	function movedo_ext_browser_webkit_check() {
...
function movedo_ext_vc_disable_updater() {
	...
	if ( function_exists( 'vc_plugin_name' ) && function_exists( 'movedo_grve_vc_updater_notification' ) ) {
		add_action( 'in_plugin_update_message-' . vc_plugin_name(), 'movedo_grve_vc_updater_notification', 11 );
	}
}
add_action( 'admin_init', 'movedo_ext_vc_disable_updater', 99 );
...
if ( version_compare( get_bloginfo('version'), '5.5', '>=' ) ) {
	add_filter( 'movedo_ext_the_content', 'wp_filter_content_tags' );
} else {
