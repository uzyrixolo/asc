	...
	function movedo_ext_vce_add_align() {
		return array(
			...
			"value" => array(
				...
				esc_html__( "Inherit", "movedo-extension" ) => 'inherit',
			),
			"description" => '',
		);
	}
...
if( !function_exists( 'movedo_ext_vce_get_heading_increase_reset' ) ) {
	function movedo_ext_vce_get_heading_increase_reset() {
		return	array(
			"type" => "dropdown",
			"heading" => esc_html__( "Reset Heading Size", "movedo-extension" ),
			"param_name" => "increase_heading_reset",
			"value" => array(
				esc_html__( "Never", "movedo-extension" ) => '',
				esc_html__( "on Small Desktop and below", "movedo-extension" ) => 'desktop-sm',
				esc_html__( "on Tablet Landscape and below", "movedo-extension" ) => 'tablet',
				esc_html__( "on Tablet Portrait and below", "movedo-extension" ) => 'tablet-sm',
				esc_html__( "on Mobile Ladscape and below", "movedo-extension" ) => 'mobile',
			),
			"description" => esc_html__( "Select if you want to reset the header size in some devices in case size is too large.", "movedo-extension" ),
...
function movedo_ext_vce_get_button_params( $group = 'button', $index = '' ) {
	...
	$btn_params = array(
		...
		array(
			"type" => "textfield",
			"heading" => esc_html__( "Button ID", "movedo-extension" ),
			"param_name" => "button" . $index . "_id",
			"description" => esc_html__( "Please make sure the ID is unique and not used elsewhere on the page.", "movedo-extension" ),
			"group" => $group_string,
		),
	);
	return $btn_params;
